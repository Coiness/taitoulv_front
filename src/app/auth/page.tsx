...
export default function AuthPage() {
    ...
    async function Forget(useremail: string, password: string) {
        if (useremail === "" || password === "") {
            alert("请输入邮箱和新密码");
            return;
        }
        ...
        try {
            const response = await AuthAPI.password_forgot(useremail, password);
            alert("密码重置成功，请重新登录");
            console.log("密码重置成功", response);
            setModel("login");
        } catch (error) {
            // 错误已在拦截器中处理，这里可以添加特定的组件级处理
            console.error("忘记密码组件中捕获到错误:", error);
            // 由于拦截器已经显示了错误消息，这里省略
            // alert("密码重置失败，请稍后再试");
        }
    }
            ...
            <div className="max-w-7xl w-full flex rounded-xl shadow-lg overflow-hidden">
                ...
                <div className="hidden md:block md:w-1/2 bg-gradient-to-r from-blue-500 to-indigo-600 p-10 flex-col justify-center">
                    <div className="text-center">
                        ...
                        <div className="mt-10 *:text-white text-l" >
                            <p>我是Logo</p>
                            <p>I am Logo</p>
                            <p>私はLogoです</p>
                            <p>저는Logo입니다</p>
                            <p>Ich bin das Logo.</p>
                            <p>Je suis le logo.</p>
                        </div>
                    </div>
                </div>
